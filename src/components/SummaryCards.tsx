    ...
    <div className="card p-4 w-full">
      <div className="flex items-center justify-between">
        <div>
          ...
          {loading ? (
            <div className="mt-1"><span className="spinner-dots"><span className="dot"/><span className="dot"/><span className="dot"/></span></div>
          ) : (
            <div className="mt-1 flex flex-wrap items-baseline gap-x-4 gap-y-1">
              <span className="text-xs text-slate-500">Tour Spontanei</span>
              <span className="text-xl font-semibold">{data?.base?.tour_spontanei ?? 0}</span>

              <span className="text-xs text-slate-500">Miss</span>
              <span className="text-xl font-semibold">{data?.base?.miss ?? 0}</span>

              <span className="text-xs text-slate-500">Venduti</span>
              <span className="text-xl font-semibold">{data?.base?.venduti_total ?? 0}</span>
            </div>
          )}
        </div>
        <button className="btn" onClick={onToggle}>{isOpen ? "Chiudi" : "Dettaglio"}</button>
      </div>

      {isOpen && !loading && (
        <div className="mt-3">
          <div className="text-sm text-slate-500 mb-2">Abbonamenti venduti per tipologia</div>
          {data && data.tipi && data.tipi.length>0 ? (
            <ul className="list-disc ml-5">
              {data.tipi.map(t => (
                <li key={`${t.tipo_id}-${t.tipo_nome}`} className="text-sm">
                  {t.tipo_nome} — <b>{t.quanti}</b>
                </li>
              ))}
            </ul>
          ) : (
            <div className="text-sm text-slate-500">Nessun venduto nel periodo.</div>
          )}
        </div>
      )}
    </div>
